...
<div id="content">
<?php
$str="select *from  tb_contact order by con_id desc"; 
...
while ($sr=$result->fetch_array()) {
	?>
                                            <tr>
                                                <td><?=$sr[con_id];?></td>
                                                <td><?=$sr[name];?></td>
                                                <td><a href="mail_detail.php?id=<?=$sr['con_id']?>">ดูรายละเอียด</a></td>
                                                <td><?=status_mail($sr[status]);?></td>
                                                <td>
<input type=checkbox id="check<?=$count?>" name="action_id[]" value="<?=$sr['con_id']?>" onClick="if(this.checked==true){ selectRow('row<?=$count?>'); }else{ deselectRow('row<?=$count?>'); }">  </td>
                                            </tr>
<? }?></div>
                                        </tbody>
                                    </table>
...
เลือกทำรายการ :  <select name="action_delmail">
