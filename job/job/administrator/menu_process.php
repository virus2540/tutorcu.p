...
if($_POST["action"]=="del_data"){
	for($i=0;$i<count($action_id);$i++){
		$pp_id=$action_id[$i];
		// ลบรูปเเสดงเนื้อหา///
		/*	$img="SELECT *  FROM tb_file WHERE gallery_id='$pp_id'";
			$resultimg=mysql_query($img);
			$imagenews=mysql_fetch_object($resultimg);
			
			if ($imagenews->file_name!="") {
				unlink("../upload/gallery/pic/$imagenews->file_name"); // Unlink File
				unlink("../upload/gallery/thumbnail/$imagenews->file_name"); // Unlink File
			}*/
	mysql_query("Delete from tb_menufood Where menu_id='$pp_id' ");
echo"<meta http-equiv=refresh content=1;url=../administrator/menufood.php>";
	}
}
?>
			<div class="success-main">
