...
if($_POST["action"]=="del_data"){
	for($i=0;$i<count($action_id);$i++){
		$pp_id=$action_id[$i];
		mysql_query("Delete from tb_payment Where payment_id='$pp_id' ");
echo"<meta http-equiv=refresh content=1;url=../administrator/payment.php>";
	}
}elseif($_POST["action"]=="yes"){
	for($i=0;$i<count($action_id);$i++){
		$pp_id=$action_id[$i];
		mysql_query("update tb_payment set status='yes' Where payment_id='$pp_id' ");
echo"<meta http-equiv=refresh content=1;url=../administrator/payment.php>";
	...
	for($i=0;$i<count($action_id);$i++){
		$pp_id=$action_id[$i];
		mysql_query("update  tb_payment set status='no' Where payment_id='$pp_id' ");
echo"<meta http-equiv=refresh content=1;url=../administrator/payment.php>";

	}
}
if($_POST["action_delmail"]=="del_data"){
	for($i=0;$i<count($action_id);$i++){
		$pp_id=$action_id[$i];
mysql_query("Delete from tb_contact Where con_id='$pp_id' ");
echo"<meta http-equiv=refresh content=1;url=../administrator/contact.php>";
