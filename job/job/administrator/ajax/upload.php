...
function Repicname($linkstrip) {
$linkstrip = ereg_replace('_+', '_', str_replace(array(' ', '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '-', '+', '|', '\\', '\'', '"', '[', ']', '{', '}', ':', ';', ',', '/', '?', '', '<', '>'), '_', trim(ereg_replace('[[:space:]]+', ' ', trim($linkstrip)))));
	...
	foreach ($_FILES['files']['name'] as $f => $name) {     
	    ...
	    if ($_FILES['files']['error'][$f] == 0) {	           
			...
			elseif( ! in_array(pathinfo($name, PATHINFO_EXTENSION), $valid_formats) ){
				...
				$name1=Repicname($name);
				$new_file=getFileNameUploadNew($name1,$path);
...
$user_id=$_SESSION["adminID"];
					$sql="INSERT INTO images (img,user_id)VALUES('$new_file','$user_id')";
					mysql_query($sql);
