...
if($_POST["action"]=="del"){
	for($i=0;$i<count($action_id);$i++){
		$pp_id=$action_id[$i];
		$conn->query("Delete from tb_category Where category_id='$pp_id' ");
echo"<meta http-equiv=refresh content=1;url=../administrator/category.php>";
	}
}
if($_POST["action"]=="del_arcate"){
	for($i=0;$i<count($action_id);$i++){
		$pp_id=$action_id[$i];
		$conn->query("Delete from tb_catearticle Where category_id='$pp_id' ");
echo"<meta http-equiv=refresh content=1;url=../administrator/article-category.php>";
	}
}
?>
			<div class="success-main">
