...
<div style="float:right;margin-bottom:10px"> <input type="button" class="da-button blue large" value="เพิ่มรายวิชาติวใหม่" onclick="window.location='./add-category.php'" /></div>
  								<div class="clear"></div>
<div id="da-ex-tabs">
                                    <ul>
   ...
   <div id="tabs-1">
								...
								<div class="clear"></div>
<form class="da-form" name=frmList  action="../administrator/category_process.php?task=yes" method=post>
...
<div id="content">
<?php
$str="select *from tb_category order by category_id asc"; 

$result=$conn->query($str); 
while ($sr=$result->fetch_array()) {
	?>
                                            <tr>
                                                <td><?=$sr[category_id];?></td>
                                                <td><?=$sr[category_name];?></td>
                                                <td><?=$sr[category_titlebar];?></td>
                                                <td><a href="./edit-category.php?cate_id=<?=$sr['category_id']?>" class="table_edit">edit</a>
<input type=checkbox id="check<?=$count?>" name="action_id[]" value="<?=$sr['category_id']?>" onClick="if(this.checked==true){ selectRow('row<?=$count?>'); }else{ deselectRow('row<?=$count?>'); }"></td>
                                            </tr>
<? }?></div>
                                        </tbody>
                                    </table>
