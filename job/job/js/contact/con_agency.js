...
//global vars
	...
	var enquiryfrm = jQuery("#property_mail_agent");

	var yourname = jQuery("#property_mail_name");

	var yournameInfo = jQuery("#span_property_mail_name");

	var youremail = jQuery("#property_mail_email");

	var youremailInfo = jQuery("#span_property_mail_email");

	var frnd_comments = jQuery("#property_frnd_comments");

	var frnd_commentsInfo = jQuery("#span_property_frnd_comments");
	...
	enquiryfrm.submit(function(){
...

		{
			...
			//hideform();
			/*setTimeout("document.getElementById('property_mail_name').value = '';document.getElementById('property_mail_email').value = '';document.getElementById('inq_phone').value = '';document.getElementById('property_frnd_comments').value = '';document.getElementById('inquiry_send_success_property').innerHTML = 'Inquiry Sent Successfully!'; ",900);
	...
	{		
		...
		if(jQuery("#property_mail_name").val() == '')

		{

			yourname.addClass("error");

			yournameInfo.text("กรุณากรอกชื่อของท่าน");
...

	{
...

		{
			...
			youremailInfo.text("กรุณากรอกอีเมล์สำหรับติดต่อ");
...

	{
		...
		if(jQuery("#property_frnd_comments").val() == '')
