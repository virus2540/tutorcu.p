...
<div>
  ...
  <div class="wapper">
    <div id="nav" ><a href="<?=$url_root;?>">หน้าหลัก</a> &gt;  Tag</div>
<div>
    <div class="left_menu">
...
<div style="float:right;margin-bottom:10px"> <input type="button" class="da-button blue large" value="เพิ่ม Tag ใหม่" onclick="window.location='./add_hashtag.php'" /></div>
  								<div class="clear"></div>
<div id="da-ex-tabs">
                                    <ul>
                                        <li><a href="#tabs-1">Hashtag</a></li>
                                    </ul>
   <div id="tabs-1">
     
								<div class="clear"></div>
<form class="da-form" name=frmList  action="../administrator/hashtag_process.php?task=yes" method=post>
...
<div id="content">
<?php
$str="select * from hashtag order by id asc"; 
...
while ($sr=$result->fetch_array()) {
	?>
                                            <tr>
                                                <td><?=$sr[id];?></td>
                                                <td><?=$sr[h_tag];?></td>
                                                <td><a href="./hashtag-edit.php?tag_id=<?=$sr['id']?>" class="table_edit"><img src="../plugin/icon/File_edit32.png" alt="icon" /></a></td>
                                                <td><input type=checkbox id="check<?=$count?>" name="action_id[]" value="<?=$sr['id']?>" onClick="if(this.checked==true){ selectRow('row<?=$count?>'); }else{ deselectRow('row<?=$count?>'); }"></td>
                                            </tr>
<? }?></div>
                                        </tbody>
                                    </table>
