...
<title>จัดการประเภทวิชาติวเตอร์</title>
...
<div>
  ...
  <div class="wapper">
    <div id="nav" ><a href="<?=$url_root;?>">หน้าหลัก</a> &gt;  วิชาติวเตอร์</div>
<div>
    <div class="left_menu">
...
<div style="float:right;margin-bottom:10px"> <input type="button" class="da-button blue large" value="เพิ่มบทความใหม่" onclick="window.location='./bloger-write.php'" /></div>
  								<div class="clear"></div>
<div id="da-ex-tabs">
                                    <ul>
                                        <li><a href="#tabs-1">รายวิชา</a></li>
                                    </ul>
   <div id="tabs-1">
     
								<div class="clear"></div>
<form class="da-form" name=frmList  action="../administrator/bloger_process.php?task=yes" method=post>
...
<div id="content">
<?php
$str="select * from tb_bloger order by bloger_id asc"; 
...
while ($sr=$result->fetch_array()) {
	?>
                                            <tr>
                                                ...
                                                <td><?php  echo '<img src="data:image;base64,'.base64_encode($sr["thumbnail"] ).'" width="100px">'; ?></td>
                                                <td><a href="./bloger-edit.php?bloger_id=<?=$sr['bloger_id']?>" class="table_edit"><img src="../plugin/icon/File_edit32.png" alt="icon" /></a></td>
                                                <td><input type=checkbox id="check<?=$count?>" name="action_id[]" value="<?=$sr['bloger_id']?>" onClick="if(this.checked==true){ selectRow('row<?=$count?>'); }else{ deselectRow('row<?=$count?>'); }"></td>
                                            </tr>
<? }?></div>
                                        </tbody>
                                    </table>
